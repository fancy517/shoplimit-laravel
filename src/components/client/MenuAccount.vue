  ...
  >
    <template v-slot:activator="{ on }">
      <v-btn
        ...
        v-on="on"
      >
        <span class="black--text">{{ clientData ? clientData.name : null }}</span>
    ...
    <v-list id="list" flat dense>
      <v-subheader>{{ clientData ? clientData.email : null }}</v-subheader>
      <v-divider />
      <v-list-tile
      ...
      menuItems: [
        {
          text: 'Account',
          icon: 'account_circle',
          callback: () => this.$router.push({ name: 'client-account' })
        },
        {
          text: 'Log Out',
          icon: 'exit_to_app',
